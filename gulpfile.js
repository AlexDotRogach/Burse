...
gulp.task('watch', function () {
  ...
  gulp.watch('src/svg/svgSpriteHard/*').on('all', gulp.parallel('svgSpriteHard'))
...
gulp.task('fonts', function () {
  if (fs.readFileSync('src/sass/_fonts.sass') !== '') fs.truncateSync('src/sass/_fonts.sass', 0) // clear file
...
function createFonts(callback) {
  return gulp.src(['src/fonts/*.ttf']).pipe(callback()).pipe(gulp.dest('dist/fonts'))
}
    ...
    if (items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (c_fontname != fontname) {
          fontname.toLowerCase().includes('bold') ? (weight = 700) : (weight = 400)
          fs.appendFile(
            'src/sass/_fonts.sass',
            '@include font("' + fontname + '", "' + fontname + '", "' + weight + '", "normal")\r\n',
            cb
          )
        }
        c_fontname = fontname
